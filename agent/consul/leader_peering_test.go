...
func TestLeader_Peering_DialerReestablishesConnectionOnError(t *testing.T) {
	...
	// Re-set a certificate signed by the original CA.
	require.NoError(t, acceptingServerRestart.tlsConfigurator.UpdateAutoTLSCert(connect.TestServerLeaf(t, "dc1", ca)))
	acceptingServerRestart.tlsConfigurator.UpdateAutoTLSPeeringServerName(connect.PeeringServerSAN("dc1", connect.TestTrustDomain))
