...
func newTestServer(t *testing.T, cb func(conf *consul.Config)) testingServer {
	...
	conf.TLSConfig.GRPC.UseAutoCert = true
	...
	require.NoError(t, deps.TLSConfigurator.UpdateAutoTLSCert(connect.TestServerLeaf(t, "dc1", ca)))
	...
	port := conf.GRPCPort
	if conf.GRPCTLSPort > 0 {
		port = conf.GRPCTLSPort
	}
